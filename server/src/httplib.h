...
#define CPPHTTPLIB_THREAD_POOL_COUNT                                                               \
  ((std::max)(                                                                                     \
      8u, std::thread::hardware_concurrency() > 0 ? std::thread::hardware_concurrency() - 1 : 0))
#endif
...
public:
  ...
  std::string get_request_header_value(const std::string &key, const char *def = "",
                                       size_t id = 0) const;
  uint64_t get_request_header_value_u64(const std::string &key, uint64_t def = 0,
                                        size_t id = 0) const;
...
public:
  ...
  Result Post(const std::string &path, const Headers &headers, const char *body,
              size_t content_length, const std::string &content_type, Progress progress);
...
inline uint64_t get_header_value_u64(const Headers &headers, const std::string &key, uint64_t def,
                                     size_t id) {
...
inline uint64_t Request::get_header_value_u64(const std::string &key, uint64_t def,
                                              size_t id) const {
  return detail::get_header_value_u64(headers, key, def, id);
}

inline uint64_t Response::get_header_value_u64(const std::string &key, uint64_t def,
                                               size_t id) const {
  return detail::get_header_value_u64(headers, key, def, id);
}
...
inline void default_socket_options(socket_t sock) {
  ...
  setsockopt(sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, reinterpret_cast<const char *>(&opt),
             sizeof(opt));
#else
#ifdef SO_REUSEPORT
...
inline Server &Server::set_write_timeout(const std::chrono::duration<Rep, Period> &duration) {
  detail::duration_to_sec_and_usec(duration,
                                   [&](time_t sec, time_t usec) { set_write_timeout(sec, usec); });
  return *this;
}
...
inline Server &Server::set_idle_interval(const std::chrono::duration<Rep, Period> &duration) {
  detail::duration_to_sec_and_usec(duration,
                                   [&](time_t sec, time_t usec) { set_idle_interval(sec, usec); });
  return *this;
}
...
inline uint64_t Result::get_request_header_value_u64(const std::string &key, uint64_t def,
                                                     size_t id) const {
...
inline std::wstring u8string_to_wstring(const char *s) {
  ...
  if (wlen > 0) {
    ws.resize(wlen);
    wlen = ::MultiByteToWideChar(CP_UTF8, 0, s, len,
...
const char *get_header_value(const Headers &headers, const std::string &key, const char *def,
                             size_t id);
...
#else
  hFile_ = ::CreateFileW(wpath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
inline Error wait_until_socket_is_ready(socket_t sock, time_t sec, time_t usec) {
  ...
  auto ret =
      handle_EINTR([&]() { return select(static_cast<int>(sock + 1), &fdsr, &fdsw, &fdse, &tv); });
...
socket_t create_socket(const std::string &host, const std::string &ip, int port, int address_family,
                       int socket_flags, bool tcp_nodelay, bool ipv6_v6only,
...
#ifdef _WIN32
      setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char *>(&opt),
                 sizeof(opt));
#else
      setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const void *>(&opt),
                 sizeof(opt));
#endif
    }
...
inline const char *get_header_value(const Headers &headers, const std::string &key, const char *def,
                                    size_t id) {
  auto rng = headers.equal_range(key);
  auto it = rng.first;
  std::advance(it, static_cast<ssize_t>(id));
...
template <typename T> inline bool parse_header(const char *beg, const char *end, T fn) {
  ...
  if (p <= end) {
    ...
    auto val = case_ignore::equal(key, "Location") ? std::string(p, end)
...
inline bool read_headers(Stream &strm, Headers &headers) {
  ...
  for (;;) {
    ...
    if (!parse_header(line_reader.ptr(), end, [&](const std::string &key, std::string &val) {
...
inline std::string
make_content_range_header_field(const std::pair<size_t, size_t> &offset_and_length,
...
inline bool PathParamsMatcher::match(Request &request) const {
  ...
  for (size_t i = 0; i < static_fragments_.size(); ++i) {
    ...
    // comparison
    if (std::strncmp(request.path.c_str() + starting_pos, fragment.c_str(), fragment.length()) !=
        0) {
      return false;
    }
  ...
  if (detail::expect_content(req)) {
    // Content reader handler
    {
      ContentReader reader(
          [&](ContentReceiver receiver) {
            return read_content_with_content_receiver(strm, req, res, std::move(receiver), nullptr,
                                                      nullptr);
          },
...
inline socket_t ClientImpl::create_client_socket(Error &error) const {
  if (!proxy_host_.empty() && proxy_port_ != -1) {
    return detail::create_client_socket(
        proxy_host_, std::string(), proxy_port_, address_family_, tcp_nodelay_, ipv6_v6only_,
    ...
    if (!username.empty() && !password.empty()) {
      ...
      if (detail::parse_www_authenticate(res, auth, is_proxy)) {
        ...
        new_req.headers.insert(detail::make_digest_authentication_header(
            req, auth, new_req.authorization_count_, detail::random_string(10), username, password,
            is_proxy));
...
inline bool ClientImpl::write_content_with_provider(Stream &strm, const Request &req,
  ...
  if (req.is_chunked_content_provider_) {
    ...
    return detail::write_content_chunked(strm, req.content_provider_, is_shutting_down, *compressor,
                                         error);
  } else {
...
inline bool ClientImpl::write_request(Stream &strm, Request &req, bool close_connection,
  ...
  // Request line and headers
  {
    ...
    const auto &path_with_query =
        req.params.empty() ? req.path : append_query_params(req.path, req.params);
...
inline Result
ClientImpl::send_with_content_provider(const std::string &method, const std::string &path,
...
inline ContentProviderWithoutLength ClientImpl::get_multipart_content_provider(
  ...
  return [&, cur_item, cur_start](size_t offset, DataSink &sink) mutable -> bool {
    if (!offset && !items.empty()) {
      ...
      if (!cur_start) {
        const auto &begin =
            detail::serialize_multipart_formdata_item_begin(provider_items[cur_item], boundary);
        offset += begin.size();
        cur_start = offset;
        sink.os << begin;
      }
...
inline Result ClientImpl::Post(const std::string &path, const Headers &headers,
  ...
  return send_with_content_provider("POST", path, headers, nullptr, content_length,
                                    std::move(content_provider), nullptr, content_type, nullptr);
}
...
inline Result ClientImpl::Post(const std::string &path, const Headers &headers,
  ...
  return send_with_content_provider("POST", path, headers, nullptr, 0, nullptr,
                                    std::move(content_provider), content_type, nullptr);
}
...
inline Result ClientImpl::Patch(const std::string &path, const Headers &headers, const char *body,
                                size_t content_length, const std::string &content_type,
                                Progress progress) {
  return send_with_content_provider("PATCH", path, headers, body, content_length, nullptr, nullptr,
                                    content_type, progress);
}
...
inline void ClientImpl::set_server_certificate_verifier(std::function<bool(SSL *ssl)> verifier) {
...
inline void Client::set_server_certificate_verifier(std::function<bool(SSL *ssl)> verifier) {
