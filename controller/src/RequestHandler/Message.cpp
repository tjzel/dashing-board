...
bool Message::isInitMessage() const {
  ...
  return dataSize() == 1 && data[0] == 0x81;
}

Message::Message(const Byte format, const Byte target, const Byte source, std::vector<Byte> data)
    : format(format), target(target), source(source), data(data), checksum(calculateChecksum(format, target, source, data)) {
...
CommandLiteral OBD2Message::command() const { return CommandLiteral{mode(), pid()}; }
